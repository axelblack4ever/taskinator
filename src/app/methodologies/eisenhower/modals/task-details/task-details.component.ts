    ...
    <ion-content class="ion-padding">
      ...
      <ion-list lines="none">
        ...
        <ion-item *ngIf="task?.category_name">
          <ion-icon name="bookmark-outline" slot="start" [color]="task?.category_color || 'primary'"></ion-icon>
          <ion-label>
            <h2>Categoría</h2>
        ...
        <ion-item *ngIf="task?.tags && task.tags.length > 0">
          <ion-icon name="pricetag-outline" slot="start" color="medium"></ion-icon>
          <ion-label>
            <h2>Etiquetas</h2>
            <div class="tags-container">
              <ion-chip *ngFor="let tag of task.tags" outline size="small">
                {{ tag }}
              </ion-chip>
            </div>
          </ion-label>
...
export class TaskDetailComponent {
  @Input() task: TaskDetailResponse | null = null;
  ...
  get isImportant(): boolean {
    return this.task?.is_important || false;
  }
